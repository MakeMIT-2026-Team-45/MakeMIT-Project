...
Sweeps the servo through -20° → 80° → 180° → 80° → -20° so you can visually
confirm the servo is wired correctly before running the full drive_client.

Direction is reversed (min_dc=12.5, max_dc=2.5) so -20° physically moves CW
...
Usage:
  ...
  python3 test_servo.py --pin 18 --angles -20 30 80 130 180
"""
...
def set_angle(pwm: GPIO.PWM, angle: float, min_dc: float = 2.5, max_dc: float = 12.5) -> None:
    """Set servo to angle_deg [-20, 180] and hold for settle time."""
    angle = max(-20.0, min(180.0, angle))
    dc = min_dc + ((angle + 20.0) / 200.0) * (max_dc - min_dc)
...
def main() -> None:
    ...
    parser.add_argument("--min-dc", type=float, default=12.5,
                        help="Duty cycle for -20° (reversed: 12.5%% = CW start).")
    ...
    parser.add_argument("--angles", type=float, nargs="+", default=[-20, 80, 180, 80, -20],
                        help="Sequence of angles to sweep (default: -20 80 180 80 -20).")
    args = parser.parse_args()

    print(f"Servo test  BCM GPIO {args.pin}  (physical pin {'32' if args.pin == 12 else '?'})")
    print(f"Frequency   {args.freq} Hz   duty cycle {args.min_dc}%% (-20°) – {args.max_dc}%% (180°)")
